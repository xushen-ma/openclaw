...
assert.equal(resolveStorePath({ memoryRoot: "/tmp/ov", agentId: "ben", scope: "memory" }), "/tmp/ov/ben");
assert.equal(
  resolveStorePath({ memoryRoot: "/tmp/ov", agentId: "ben", scope: "sessions" }),
  "/tmp/ov/ben-sessions",
);

assert.deepEqual(toOpenVikingResult({ total: 2, items: [{ uri: "x", score: 0.5, abstract: "hit" }] }, 1), {
  result: {
    total: 2,
