...
import { resolveFsExtraRoots } from "./fs-root-policy.js";
...
export function createOpenClawCodingTools(options?: {
  ...
  const extraRoots = workspaceOnly ? resolveFsExtraRoots(fsPolicy.extraRoots) : [];
  ...
  const base = (createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]).flatMap((tool) => {
    ...
    if (tool.name === "write") {
      ...
      const wrapped = createHostWorkspaceWriteTool(workspaceRoot, { workspaceOnly, extraRoots });
      return [
        workspaceOnly
          ? wrapToolWorkspaceRootGuardWithOptions(wrapped, workspaceRoot, {
              extraRoots,
    ...
    if (tool.name === "edit") {
      ...
      const wrapped = createHostWorkspaceEditTool(workspaceRoot, { workspaceOnly, extraRoots });
      return [
        workspaceOnly
          ? wrapToolWorkspaceRootGuardWithOptions(wrapped, workspaceRoot, {
              extraRoots,
