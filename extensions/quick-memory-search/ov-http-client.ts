...
function buildPerAgentUrl(baseUrl: string, scope: OvSearchScope, agentId: string, routing: OvHttpConfig["agentRouting"]): string {
...
export function resolveOvRequest(opts: {
  ...
  if (config.perAgentBaseUrl) {
    ...
    requests.push({
      ...
      init: { method: "POST", headers, body: JSON.stringify(body), signal: AbortSignal.timeout(10_000) },
      mode: "per-agent",
    });
  }
