...
const QuickSessionSearchSchema = {
  ...
  properties: {
    query: { type: "string" as const, description: "Search query for past session/conversation recall" },
...
const VENV_PY = "/Users/openclaw/.openclaw/workspace/projects/openviking-migration/tmp/openviking-venv/bin/python";

function joinUrl(base: string, path: string): string {
  return `${base.replace(/\/$/, "")}/${path.replace(/^\//, "")}`;
}

function formatFailure(params: { error: string; fallback: string; failureClass?: OvFailureClass; routing?: "per-agent" | "local"; mode?: "per-agent" | "local"; status?: number; }): Record<string, string> {
...
async function searchLocalStore(storePath: string, query: string, limit: number): Promise<{ total: number; items: any[]; error?: string }> {
  const script = `
...
`;
  try {
    const { stdout } = await execFileAsync(VENV_PY, ["-c", script, storePath, query, String(limit)], {
      timeout: 15_000,
      env: { ...process.env, PYTHONDONTWRITEBYTECODE: "1" },
    });
    const parsed = JSON.parse(typeof stdout === "string" ? stdout : "{}") as { total?: number; items?: any[]; error?: string };
    return { total: parsed.total ?? 0, items: Array.isArray(parsed.items) ? parsed.items : [], error: parsed.error };
  } catch (err) {
...
export function createQuickSessionSearchTool(httpConfig: { perAgentBaseUrl: string }, agentId: string): AnyAgentTool {
  return {
    ...
    execute: async (_toolCallId: string, params: unknown) => {
      ...
      if (httpConfig.perAgentBaseUrl) {
        try {
          const response = await fetch(joinUrl(httpConfig.perAgentBaseUrl, "api/v1/search/session-find"), {
            method: "POST",
          ...
          if (response.ok) {
            ...
            return jsonResult({ results, provider: "openviking-sessions-agent-http", routing: "per-agent", agentId });
          }
      ...
      const local = await searchLocalStore(`${OV_LOCAL_ROOT}/${agentId}-sessions`, normalizedQuery, maxResults);
      if (local.error) {
        ...
        return jsonResult(formatFailure({ error: "session quick search failed", fallback: "Use sessions_history as fallback.", failureClass, routing: "local", mode: "local" }));
      }
      ...
      return jsonResult({ results, provider: "openviking-sessions-local", routing: "local", agentId });
    },
  };
}
