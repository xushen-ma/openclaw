...
export function classifyOvFailure(params: { status?: number; reason?: string }): OvFailureClass {
  ...
  if (params.status === 429 || /\b429\b/.test(normalized)) {
    return "rate";
  }
  if (tokenFailurePatterns.some((pattern) => pattern.test(normalized))) {
    return "token";
  }
  if (rateFailurePatterns.some((pattern) => pattern.test(normalized))) {
    return "rate";
  }
  if (resourceFailurePatterns.some((pattern) => pattern.test(normalized))) {
    return "resource";
  }
