...
- Upstream target: `v2026.7.1`
- Upstream peeled SHA: `2d2ddc43d0dcf71f31283d780f9fe9ff4cc04fe4`
...
- The integration branch is clean and currently points directly at upstream tag `v2026.7.1` / `2d2ddc43d0d`.
...
- Nina's `2026.7.1-2` packaging suffix is not proven equivalent to the source tag in this worktree. Treat it as decision/evidence-needed before a promotion packet.
...
| ZenMux image generation provider | Keep if Xushen/Mini still want ZenMux image generation | Recreate as a 7.1-compatible external/provider plugin. Register one `zenmux` image-generation provider, keep API-key auth choice, and adapt to current 7.1 plugin/provider interfaces. | Upstream 7.1 has official image providers (`openai`, `google`, `openrouter`, `fal`, `deepinfra`, `comfy`, `vydra`, `minimax`) but no ZenMux plugin. `@google/genai` is already present upstream at `2.10.0`; avoid downgrading or lockfile copying. |
...
| Matrix SDK install guard before account startup | Decision-needed | Check current 7.1 Matrix packaging/runtime first. If cold-start on the Mac can still start before SDK dependencies are available, replay `ensureMatrixSdkInstalled({ runtime })` before `startAccount` imports runtime. | 7.1 Matrix dependency handling changed substantially; do not replay blindly without a cold-start/plugin install test. |
...
| `tools.fs.extraRoots` for workspace-only tools | Decision-needed, security-sensitive | Only replay if Mini explicitly confirms the fleet still needs extra roots. If kept, use a 7.1-native design with `ro`/`rw` modes, canonicalization, symlink/hardlink checks, and tests for read/write/edit/apply_patch. | This widens the filesystem boundary. Security review requires plugin/tool policy not accidentally broaden exposure. Default should remain no extra roots. |
...
1. Baseline guard: keep branch rooted at `v2026.7.1` (`2d2ddc43d0d`), verify `git status --short --branch` is clean before each replay cluster.
...
5. Plugin replay:
   ...
   - create/update ZenMux as a pinned provider plugin compatible with 7.1;
...
- If `tools.fs.extraRoots` is replayed: `pnpm test -- src/agents/tool-fs-policy.test.ts src/agents/agent-tools.workspace-paths.test.ts src/agents/apply-patch.test.ts`
...
- If ZenMux is replayed: image provider registration contract, auth resolution tests, supported/unsupported model tests, and one redacted staging `image_generate` smoke.
...
- Prove Nina's `2026.7.1-2` package source mapping: exact source SHA or explicit non-equivalence.
...
- Confirm whether ZenMux is still required now that upstream 7.1 has multiple official image providers.
- Confirm whether `tools.fs.extraRoots` is still required. If yes, define exact allowed roots and modes per agent; default deny everything else.
- Decide whether Matrix SDK install guard is still necessary after a 7.1 cold-start check.
