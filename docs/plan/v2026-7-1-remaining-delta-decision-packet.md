...
- Current candidate head at packet creation: `b638c4c4c85`
...
- Focused proof under isolated `node@25.9.0` passes: 5 Vitest shards, 9 files, 353 tests.
...
| Matrix SDK install guard                 | Needs cold-start evidence                 | Do not replay blindly. 7.1 already ships Matrix plugin-local deps and `ensureMatrixSdkInstalled`; only add a startup guard if a clean cold-start/plugin-install test proves startup can still fail before repair guidance.        | Cold-start Matrix plugin evidence in a disposable state root; installed/missing dependency cases; gateway restart smoke; `openclaw doctor --fix` repair path; startup test proving failure is graceful or deps are installed before runtime import. |

## Recommended Next Sequence

1. Do not implement any remaining optional delta yet.
2. Run a disposable-state Matrix cold-start/plugin-install proof for the SDK guard.
3. Ask Xushen for product/security decisions on Quick Memory, ZenMux, and `tools.fs.extraRoots`.
4. After decisions, create separate commits for any approved plugin/security work.
5. Only then push and use governed test-lane/staging.
