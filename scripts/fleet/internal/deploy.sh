...
if [[ -f "$SCRIPT_DIR/fleet.env" ]]; then
  ...
  DEV_REPO="${DEV_REPO:-$HOME/workspace/openclaw-staging}"
  ...
  AIRI_BASE_URL="${AIRI_BASE_URL:-http://127.0.0.1:18789}"
...
if ! git merge-base --is-ancestor "$MAIN_SHA" "$LINEAGE_REF" >/dev/null 2>&1; then
  LINEAGE_HEAD="$(git rev-parse "$LINEAGE_REF" 2>/dev/null || echo unknown)"
  ON_LINEAGE=no
  git merge-base --is-ancestor "$MAIN_SHA" "$LINEAGE_REF" >/dev/null 2>&1 && ON_LINEAGE=yes || true
  echo "❌ Refusing deploy: candidate is not on the resolved lineage"
  echo ""
  echo "Reason:"
  echo "- candidate SHA: $MAIN_SHA"
  echo "- candidate on lineage: $ON_LINEAGE"
  echo "- lineage ref: $LINEAGE_REF"
  echo "- lineage head: $LINEAGE_HEAD"
  echo ""
  echo "What this means:"
  echo "- releasectl deploy is the governed production tag+deploy step"
  echo "- it only accepts candidates already present on the production lineage"
  echo "- a merged main SHA is not automatically a production deploy candidate"
  echo ""
  echo "What to do next:"
  echo "1. Promote the commit onto production first:"
  echo "     releasectl promote-production --sha <validated-tag-or-sha>"
  echo "2. Run this step again with --sha against the resulting production-lineage SHA:"
  echo "     releasectl deploy --sha <resulting-production-lineage-sha>"
  exit 1
fi
