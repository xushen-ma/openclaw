...
if [[ "$CONTROLLED_HEAD" != "$CONTROLLED_UPSTREAM" ]]; then
  ...
  echo "FAST-FORWARDED controlled repo to $(git -C "$CONTROLLED_REPO" rev-parse HEAD)"
fi
...
if [[ -d "$EXPORT_DIR/$SOURCE_BUNDLE_PATH/internal" ]]; then
  for src in "$EXPORT_DIR/$SOURCE_BUNDLE_PATH/internal"/*; do
    [[ -e "$src" ]] || continue
    IMPORT_MANIFEST+=("internal/$(basename "$src")")
  done
fi

if [[ "$DRY_RUN" -eq 1 ]]; then
  DRY_RUN_WORKSPACE="$(mktemp -d)"
  cp -R "$CONTROLLED_REPO/.git" "$DRY_RUN_WORKSPACE/"
  [[ -d "$CONTROLLED_REPO/bin" ]] && cp -R "$CONTROLLED_REPO/bin" "$DRY_RUN_WORKSPACE/"
  [[ -d "$CONTROLLED_REPO/internal" ]] && cp -R "$CONTROLLED_REPO/internal" "$DRY_RUN_WORKSPACE/"
  WORK_CONTROLLED="$DRY_RUN_WORKSPACE"
else
  WORK_CONTROLLED="$CONTROLLED_REPO"
fi

source_path_for_import() {
  case "$1" in
    bin/releasectl)
      echo "$SOURCE_REPO/$SOURCE_BUNDLE_PATH/releasectl"
      ;;
    internal/*)
      echo "$SOURCE_REPO/$SOURCE_BUNDLE_PATH/internal/${1#internal/}"
      ;;
    *)
      echo ""
      ;;
...
for rel in "${IMPORT_MANIFEST[@]}"; do
  SRC_PATH="$(source_path_for_import "$rel")"
  if [[ -z "$SRC_PATH" ]]; then
    continue
  fi
  DEST_PATH="$WORK_CONTROLLED/$rel"
  mkdir -p "$(dirname "$DEST_PATH")"
  cp "$SRC_PATH" "$DEST_PATH"
  if [[ "$rel" == bin/releasectl || "$SRC_PATH" == *.sh ]]; then
...
git -C "$WORK_CONTROLLED" add "${IMPORT_MANIFEST[@]}" >/dev/null 2>&1 || true
...
git -C "$CONTROLLED_REPO" commit -m "$COMMIT_MSG"
IMPORT_COMMIT="$(git -C "$CONTROLLED_REPO" rev-parse HEAD)"
echo "COMMITTED sha=$IMPORT_COMMIT"

echo "PUSH remote=$CONTROLLED_REMOTE branch=$CONTROLLED_BRANCH"
git -C "$CONTROLLED_REPO" push "$CONTROLLED_REMOTE" "$CONTROLLED_BRANCH"
echo "PUSHED"
...
for rel in "${IMPORT_MANIFEST[@]}"; do
  case "$rel" in
    bin/releasectl)
      ...
      ;;
    internal/*)
      SRC_PATH="$SOURCE_REPO/$SOURCE_BUNDLE_PATH/internal/${rel#internal/}"
      ;;
    *)
      echo "VERIFY-ERROR unknown mapping: $rel" >&2
      VERIFY_STATUS=1
  ...
  CTL_PATH="$CONTROLLED_REPO/$rel"

  if [[ ! -f "$SRC_PATH" ]]; then
    echo "VERIFY-MISSING-SOURCE $rel"
    VERIFY_STATUS=1
    VERIFY_DIFF=$((VERIFY_DIFF + 1))
    continue
  fi
  ...
  if [[ ! -f "$CTL_PATH" ]]; then
    echo "VERIFY-MISSING-CONTROLLED $rel"
    VERIFY_STATUS=1
    VERIFY_DIFF=$((VERIFY_DIFF + 1))
    continue
  fi
  ...
  if cmp -s "$SRC_PATH" "$CTL_PATH"; then
    ...
    echo "VERIFY-OK $rel sha=$SRC_SHA"
    VERIFY_OK=$((VERIFY_OK + 1))
  else
    SRC_SHA="$(shasum -a 256 "$SRC_PATH" | awk '{print $1}')"
    CTL_SHA="$(shasum -a 256 "$CTL_PATH" | awk '{print $1}')"
    echo "VERIFY-DIFF $rel src_sha=$SRC_SHA ctl_sha=$CTL_SHA"
    VERIFY_STATUS=1
    VERIFY_DIFF=$((VERIFY_DIFF + 1))
  fi
done
