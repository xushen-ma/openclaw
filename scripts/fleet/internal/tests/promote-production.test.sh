...
repeat_out="$({
  ...
  export MAIN_BRANCH="main"
  export PROD_BRANCH="production"
  bash "$PROMOTE_SCRIPT" --sha "$REMOTE_PROD_SHA"
} 2>&1)" || fail "promote-production should be idempotent for lineage-head commit"

[[ "$repeat_out" == *"Candidate is already on production lineage"* ]] || fail "expected idempotent production-lineage output"
[[ "$repeat_out" == *"result_sha=$REMOTE_PROD_SHA"* ]] || fail "expected repeat promotion to return existing production SHA"
