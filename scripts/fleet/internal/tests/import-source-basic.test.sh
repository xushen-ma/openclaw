...
# Run import-source
export RELEASECTL_SOURCE_REPO="$SOURCE_REPO"
export RELEASECTL_CONTROLLED_REPO="$CONTROLLED_REPO"
export RELEASECTL_CONTROLLED_REMOTE="origin"
export RELEASECTL_CONTROLLED_BRANCH="main"

OUTPUT="$("$SCRIPT_DIR/import-source.sh" "$SOURCE_SHA" 2>&1)" || {
  echo "FAIL import-source exited non-zero"
  echo "$OUTPUT"
  exit 1
}
...
if [[ ! -f "$CONTROLLED_REPO/bin/releasectl" ]]; then
  echo "FAIL bin/releasectl not imported"
  exit 1
fi

if [[ ! -f "$CONTROLLED_REPO/internal/test.sh" ]]; then
  echo "FAIL internal/test.sh not imported"
  exit 1
fi

if [[ ! -f "$CONTROLLED_REPO/internal/fleet.env" ]]; then
  echo "FAIL internal/fleet.env not imported"
  exit 1
fi
