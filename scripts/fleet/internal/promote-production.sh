...
LINEAGE_REF="${FLEET_LINEAGE_REF:-$PROD_REF}"
...
git rev-parse --verify "$LINEAGE_REF" >/dev/null 2>&1 || {
  echo "❌ Missing production lineage reference: $LINEAGE_REF"
  exit 1
}
...
LINEAGE_HEAD="$(git rev-parse "$LINEAGE_REF")"

if git merge-base --is-ancestor "$TARGET_SHA" "$LINEAGE_HEAD" >/dev/null 2>&1; then
  echo "✅ Candidate is already on production lineage"
  echo "candidate_sha=$TARGET_SHA"
  echo "result_sha=$TARGET_SHA"
  echo "next: releasectl deploy --sha $TARGET_SHA"
  exit 0
fi
